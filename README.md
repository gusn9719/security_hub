...
**다신호 휴리스틱 기반 피싱 위험 분석 및 가상 환경 테스트 앱** — 한이음 ICT 드림업 2026
...
또 하나의 핵심은 **피드백 순환 구조**입니다. 사용자가 가상 샌드박스에서 직접 확인한 뒤 남긴 안전/위험/스팸 투표가 누적되어 다음 분석의 휴리스틱 시그널로 즉시 반영됩니다 — 한 번 판정하고 끝나는 도구가 아니라, C-TAS가 아직 못 잡은 신규 위협을 사용자 참여로 발견하고 후속 분석에 반영하는 구조입니다.
...
| [docs/user_flow.md](docs/user_flow.md) | 사용자 흐름 · 화면 구성도 |

### 평가·테스트

| 문서 | 내용 |
|---|---|
| [docs/system_accuracy_eval.md](docs/system_accuracy_eval.md) | 시스템 판정 정확도 (C-TAS 탐지율, 오탐률) |
| [docs/heuristic_eval.md](docs/heuristic_eval.md) | 휴리스틱 스코어러 단위 분석 |
| [docs/sandbox_eval.md](docs/sandbox_eval.md) | 샌드박스 실데이터 평가 |
| [docs/load_test.md](docs/load_test.md) | 부하 테스트 |
| [docs/qa_benchmark.md](docs/qa_benchmark.md) | 응답시간·동시성 등 비기능 요구사항 실측 |
| [docs/security_audit.md](docs/security_audit.md) | 보안 감사 |
| [docs/sandbox_hardening.md](docs/sandbox_hardening.md) | 샌드박스 격리 강화 및 컨테이너 탈출 시도 검증 |
