...
_KASM_AUTH_HEADER = (
    ...
    + base64.b64encode(f"kasm_user:{VNC_PW}".encode())
    + b"\r\n"
)
...
async def _proxy_handle(
    ...
    if is_websocket:
        ...
        # 일반 HTTP: kasmweb 응답 전체를 버퍼에 수신한 뒤
        # Content-Length를 실제 바이트 수로 재작성해 전송한다.
        # 스트리밍 릴레이 시 SSL 연결 종료 타이밍 차이로 선언된
        # Content-Length보다 적은 바이트가 전달돼 ERR_CONTENT_LENGTH_MISMATCH가
        # 발생하는 문제를 방지한다.
        resp_buf = b""
        try:
            while True:
        ...
        if b"\r\n\r\n" in resp_buf:
            ...
            resp_lines.append(f"Content-Length: {len(resp_body)}".encode())
            resp_headers_raw = b"\r\n".join(resp_lines)
...
async def _run_proxy_server(
    ...
    try:
        ...
        server.close()
        logger.info("[proxy] 프록시 종료: 0.0.0.0:%d → 127.0.0.1:%d", proxy_port, target_port)
...
async def _start_ssl_strip_proxy(target_port: int) -> tuple[int, asyncio.Task]:
    ...
    logger.info(
        "[proxy] HTTP-aware 프록시 시작: 0.0.0.0:%d → 127.0.0.1:%d (Auth 주입: kasm_user:***)",
        proxy_port,
        target_port,
    )
...
async def create_browse_session(
    ...
    """
    kasmweb/chromium 컨테이너를 생성하고 KasmVNC noVNC URL을 반환한다.
    ...
    try:
        ...
        try:
            kiosk_script = (
                ...
                "  if [ -n \"$CPID\" ]; then "
                "    BIN=$(readlink -f /proc/$CPID/exe 2>/dev/null); "
                "    [ -z \"$BIN\" ] && BIN=$(command -v chromium"
        ...
        proxy_port, proxy_task = await _start_ssl_strip_proxy(int(host_port))

        container_id = container.id
        # resize=remote: 접속 시 서버가 X 디스플레이 해상도를 클라이언트
        # WebView의 CSS 픽셀 크기(≈411px)로 자동 조정한다.
        # RESOLUTION 환경변수가 kasmweb에서 무시될 경우의 폴백으로도 동작한다.
        # 재조정은 최초 연결 시 1회만 발생하므로 지속적 렌더링 부하가 없다.
        # quality=6 + compression=2: 로컬호스트 기준 CPU 인코딩 부하 최소화.
        novnc_url = (
            f"http://10.0.2.2:{proxy_port}/"
            f"?password={VNC_PW}&autoconnect=1&reconnect=1"
        ...
        logger.info(
            "[browse] 세션 생성 완료: container=%s, proxy=%d, url=%s",
            container_id[:12], proxy_port, novnc_url,
        )
        return {
